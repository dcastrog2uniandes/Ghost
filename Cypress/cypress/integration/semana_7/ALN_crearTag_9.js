...
let userData = {
  ...
  color:  faker.internet.color(100, 100, 100).substring(1,9),
        ...
        //Redirected to dashboard
          ...
          cy.get('form').within(() => {
            ...
            cy.get('div.input-color').within(() => {
               cy.get('.gh-input').type(userData.color)
            })
             cy.get('#tag-slug').type(userData.cadena)
             cy.get('#tag-description').type(userData.cadena)
          })
          cy.wait(500)
          //Click save tag
         ...
         cy.get('span.error').should('contain', 'You must specify a name for the tag')
